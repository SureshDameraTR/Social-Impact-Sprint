...
- **8 migration versions**: initial -> performance_indexes -> otp_table -> reference_data -> float_to_numeric -> audit_softdelete -> gender_column -> vet_consultations
- **Artifacts**: None
- **Baselines**: None
...
From `reports/latest/workspace-audit-2026.md` (147 findings):

| Severity | Count | Top Categories |
|----------|-------|----------------|
| CRITICAL | 26 | SEC (8 unprotected endpoints, .env secrets, localStorage token, CSP hardcoded, ILIKE wildcards, BaseHTTPMiddleware), DATA (hard delete, no soft-delete filtering, missing mixin, no ON DELETE, float for money, migration chain broken, unbounded cache), MOB (onboarding never persisted x2, vaccination false positive, no delete confirmation, raw fetch login) |
| HIGH | 38 | PERF (N+1 IoT 50 calls, eager loading, in-memory aggregation x3, 9 sequential queries, no HTTP pooling, no retry, recharts in render, client pagination), SEC (no rate limiting, JWT 24h no refresh, file upload no validation, mock 0.0.0.0, CSRF unbound, no cache invalidation, unvalidated URLs, unsalted Aadhaar hash), DATA (no unique yield_logs, no buffalo disease rules, wrong IoT params, 6 raw list endpoints, missing updated_at, invalid mock date, only 12/31 districts), TEST (0 auth tests, 0 CSRF tests, 0 mobile tests, duplicate migration IDs), FE (all "use client", double API prefix, client sort after server paginate, missing print.css) |
| MEDIUM | 46 | Code quality (9), Architecture (9), Mobile (11), Frontend (8), Accessibility (9) |
| LOW | 37 | Style, consistency, tooling, optimization opportunities |

**Priority action plan**: 4 phases over 4 weeks — Stop the Bleeding (7 tasks) -> Harden (9) -> Optimize (8) -> Polish (8)
...
### Workspace Audit Critical Items (26)
13. 8+ endpoints without auth (SEC-01)
14. Animal hard delete (DATA-01)
15. Soft-delete filtering missing on ALL queries (DATA-02)
16. Migration chain broken — duplicate revision IDs (DB-01)
17. Mobile onboarding data never persisted (MOB-01, MOB-02)
18. Vaccination catch block applies same update on failure (MOB-03)
