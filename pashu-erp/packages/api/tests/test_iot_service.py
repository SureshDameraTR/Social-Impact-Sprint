...
from app.services.iot_service import list_devices, get_device, get_latest_telemetry, get_telemetry


def _mock_client(response_json, method="get"):
    """Helper to build a mock httpx.AsyncClient with a canned response."""
    mock_resp = MagicMock()
...
class TestListDevices:
    ...
    async def test_returns_device_list(self):
        payload = {"data": [{"device_id": "D1", "type": "temperature"}], "total": 1}

        with patch("app.services.iot_service.settings") as s, \
             patch("app.services.iot_service.httpx.AsyncClient", return_value=_mock_client(payload)):
    ...
    async def test_filters_passed_as_params(self):
        ...
        with patch("app.services.iot_service.settings") as s, \
             patch("app.services.iot_service.httpx.AsyncClient", return_value=mc):
            s.iot_gateway_url = "http://mock:8001/iot"
        ...
        assert call_kwargs.kwargs["params"]["device_type"] == "gps"
...
class TestGetTelemetry:
    ...
    async def test_returns_telemetry_data(self):
        payload = {"data": [{"metric": "temperature", "value": 38.5}], "total": 1}
